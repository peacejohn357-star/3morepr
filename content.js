...
(function () {
    ...
    el.innerHTML = `
      ...
      <div id="tt-body">
        ...
        <div id="tt-config">
          ...
          <div class="tt-config-row">
            <label>Epsilon</label>
            <input type="number" id="tt-cfg-epsilon" min="0" max="1" step="0.01" value="0.2">
          </div>
          <div class="tt-config-row">
  ...
  function bindButtons(el) {
    document.getElementById('tt-min-btn').addEventListener('click', () => {
    ...
    document.getElementById('tt-config-toggle').addEventListener('click', () => {
    ...
    document.getElementById('tt-cfg-epsilon').addEventListener('change', function () {
      cfg.epsilon = parseFloat(this.value) || 0.2;
      saveCfg();
    });
  ...
  function handleTick(tick) {
    ...
    signals.forEach(sig => {
      if (sig.result === 'PENDING') sig.ticksAfter.push(price);
    });
  }
  ...
  function detectSignal() {
    ...
    const isEarly = streak <= 2;
    const isLate = streak >= 5;
    ...
    if (res) {
      ...
      if ([3, 4].includes(streak)) res = null;
      else if (Math.abs(t0.deltaChange) <= eps) res = null;
      else if (t0.absSpeed > sLow && t0.absSpeed < sHigh && Math.abs(t0.speedTrend) < 0.001) res = null;
      ...
      if (res) {
        ...
        let conf = res.conf;
        if ((res.type === 'BUY' && !buyDigitBias) || (res.type === 'SELL' && !sellDigitBias)) conf -= 10;
  ...
  // ── UI & Real Execution Infrastructure ────────────────────────────────────
  ...
  function updateSignalsUI() {
    ...
    signals.slice(-10).reverse().forEach(sig => {
      ...
      div.className = `tt-signal tt-signal-${sig.type.toLowerCase()}`;
  ...
  function exportRealCSV() {
    ...
    const blob = new Blob([csv], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = '3tick-real.csv'; a.click();
  }
  ...
  function loadCfg() {
    ...
    return Object.assign({ strategyMode: 'hybrid', epsilon: 0.2, realTradeEnabled: false, realTimeoutMs: 40000, realCooldownMs: 5000, postTradeCooldownTicks: 5, postTradeCooldownMs: 5000, debugSignals: true }, stored || {});
  }
  ...
  function applyConfigToUI() {
    ...
    if (dbg) dbg.checked = cfg.debugSignals; if (re) re.checked = !!cfg.realTradeEnabled; if (mode) mode.value = cfg.strategyMode; if (eps) eps.value = cfg.epsilon;
    updateRealUI();
  }
  ...
  function startWatchdog() {
    ...
    watchdogInterval = setInterval(() => {
      ...
      if (lastTickProcessedAt > 0 && now - lastTickProcessedAt > WATCHDOG_TICK_TIMEOUT) {
        if (ws) ws.close();
        scheduleReconnect();
      }
    }, WATCHDOG_INTERVAL);
  ...
  function finalizeRealTrade(res) {
    ...
    clearTimeout(realExecTimer); realExecTimer = null;
    updateRealUI();
  }
  ...
  async function executeRealTrade(side) {
    ...
    try {
      ...
      realTrades.push({ time: Date.now(), signal: side, side: buyLabel, result: 'PENDING' });
  ...
  function simulateExternalClick(el) {
    ...
    el.focus(); el.dispatchEvent(new MouseEvent('mouseup', opts)); el.dispatchEvent(new MouseEvent('click', opts));
